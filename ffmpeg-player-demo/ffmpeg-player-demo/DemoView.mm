...
-(void)initVideo:(NSString *)videoPath{
    ...
    [[NSApp mainWindow] setFrame:NSRectFromCGRect(CGRectMake(100, 100, pCodecCtx->width, pCodecCtx->height)) display:YES];
    
    timer = [NSTimer timerWithTimeInterval:0.9/fps repeats:YES block:^(NSTimer * _Nonnull timer) {
        /// 根据视频的fps解码渲染视频
